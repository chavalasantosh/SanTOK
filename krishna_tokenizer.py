...
def _bpe_like_split(word):
    ...
    # Simulate BPE merging with common patterns
    merged_tokens = _merge_common_patterns(tokens)
    return merged_tokens


def _merge_common_patterns(tokens):
    """Merge common character patterns found in English"""
    ...
    while i < n:
        ...
        for prefix in prefixes:
            if i + _len(prefix) <= n:
                if _join_tokens(tokens, i, i + _len(prefix)) == prefix:
        ...
        if not merged:
            ...
            for suffix in suffixes:
                if i + _len(suffix) <= n:
                    if _join_tokens(tokens, i, i + _len(suffix)) == suffix:
...
def _join_tokens(tokens, start, end):
    """Join tokens from start to end index"""
    result = ""
...
def _frequency_based_split(word):
    """
    Frequency-based splitting using common English patterns
    """
    if _len(word) <= 2:
        return [word]
    ...
    common_patterns = [
        "th", "he", "in", "er", "an", "re", "ed", "nd", "on", "en", "at", "ou", "it", "is", "or", "ti", "as", "to", "be", "we", "ha", "hi", "do", "no", "if", "up", "my", "go", "me", "so", "us", "am", "by", "of", "on", "or", "to", "up", "we"
    ]
    ...
    while i < n:
        ...
        for pattern in common_patterns:
            if i + _len(pattern) <= n and word[i:i + _len(pattern)] == pattern:
...
def _reconstruct_subword_tokens(tokens):
    ...
    # Group tokens by parent word
    word_groups = {}
    for token in tokens:
        parent_start = token.get("parent_start", 0)
        if parent_start not in word_groups:
            word_groups[parent_start] = []
        word_groups[parent_start].append(token)
    
    # Reconstruct each word from its subwords
    result = ""
    for parent_start in sorted(word_groups.keys()):
        word_tokens = word_groups[parent_start]
        # Sort by subword_index
        word_tokens.sort(key=lambda t: t.get("subword_index", 0))
        
        # Reconstruct word from subwords
        word = "".join(t["text"] for t in word_tokens)
        result += word
